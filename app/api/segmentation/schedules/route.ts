...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await aepClient.getSegmentSchedules(limit)

    return NextResponse.json(data)
  } catch (error) {
    console.error("Failed to fetch segment schedules:", error)
    return NextResponse.json({ error: "Failed to fetch segment schedules" }, { status: 500 })
  }
}
